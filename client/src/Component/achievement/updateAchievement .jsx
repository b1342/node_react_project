...
const UpdateAchievement  = ({ achievementt ,setChange}) => {
  console.log(`achievementt`, achievementt);
  const { token } = useSelector((state) => state.token);

  const [visible, setVisible] = useState(false);
  const [achievement, setAchievement] = useState(achievementt.achievement);
  const [date, setDate] = useState(achievementt.date);
  ...
  const handleAddAchievement = async () => {
    ...
    // try {
    console.log(`achievementt`, achievementt);
      await achievementServices.updateAchivement(
  {
    _id: achievementt._id,
    achievement,
    date
  },
  token
  ...
  return (
    <>
      <Button label="עדכן הישג" icon="pi pi-plus" onClick={() => setVisible(true)} />
